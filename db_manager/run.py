...
def start_refresh_duration_curve_hourly_raw_local_nightly(
    hour=HOURLY_RAW_DURATION_CURVE_MV_REFRESH_HOUR,
    minute=HOURLY_RAW_DURATION_CURVE_MV_REFRESH_MINUTE,
    tz_name=HOURLY_RAW_DURATION_CURVE_MV_REFRESH_TZ,
):
    # runs refresh_mv_flow_duration_curve_hourly_raw_local nightly
    def loop():
        tz = pytz.timezone(tz_name)
        i = 1
        while True:
            try:
                ...
                sleep_seconds = (target - now).total_seconds()
                print(
                    f"Refresh duration_curve_hourly_raw_local job {i} sleeping for {sleep_seconds}."
                )
                sleep(sleep_seconds)

                refresh_mv_flow_duration_curve_hourly_raw_local()
                print(
                    f"Refresh duration_curve_hourly_raw_local job {i} executed successfully."
                )
                i += 1
            except Exception as e:
                print(
                    f"Error executing duration_curve_hourly_raw_local job {i}: {e}"
                )
                sleep(60)
    print(
